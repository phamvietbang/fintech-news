...
class MongoDB:
    ...
    def get_docs(
            self, collection, keys: list = None, filter_: dict = None, batch_size=1000, projection=None):
        # change filter_ to obj
        filter_statement = {}
        if keys:
    ...
    def get_doc(self, collection, key: str = None, filter_: dict = None, batch_size=1000, projection=None):
        # change filter_ to obj
        filter_statement = {}
        if key:
    ...
    def get_docs_with_db(
            self, db, collection, keys: list = None, filter_: dict = None, batch_size=1000,
    ...
    def update_docs(self, collection_name, data, keep_none=False, merge=True, shard_key=None, flatten=True):
        ...
        if not flatten:
            ...
            for idx in range(0, len(bulk_operations), 1000):
                update_data = bulk_operations[idx:idx + 1000]
                col.bulk_write(update_data)
            return
            ...
            if shard_key:
                ...
                bulk_operations += [
                    UpdateOne(
                        ...
                        {"$unset": {key: value for key, value in item.items() if key not in keys}}, upsert=True
                    )
                    for item in unset]
                bulk_operations += [
                    UpdateOne(
                ...
                bulk_operations += [
                    UpdateOne(
                        {"_id": item["_id"], shard_key: item[shard_key]},
                        {"$addToSet": {key: value for key, value in item.items() if key not in keys}}, upsert=True
                    )
                    for item in add_to_set]
